...
提供三种通知模式：`sync`（构造时同步调用，无丢失风险，适合日志/指标）、`async_queue`（入队后台消费，队列满时拒绝，适合网络上报/持久化）、`sync_deferred`（线程本地缓冲，显式 `flush_deferred_notifications()` 触发，缓冲满时丢弃，适合请求批处理/事务边界累积）。选择决策详见 [决策树 · 1](decision_tree.md#1-通知模式选择)。
...
> **注意**：单例用 `std::call_once` + 函数局部静态，跨 TU 安全；但请勿在其它 TU 的静态初始化代码中查询注册表。
...
`i18n_t` 单例维护 `locale_t → (code identity → message)` 两级哈希，查询路径：active → default → 空字符串。`subsystem_module_catalog_t` 维护 `(subsystem_id, module_id) → (locale → 名称)` 两级映射，与 `i18n_t` 共用 `locale_t` 但独立存储，避免双源不同步。`subsystem_module_catalog_t` 不持有 locale 状态，查询时由调用方（如 `error_context_serializer_t`）显式传入 locale。
...
> **注意**：`ERROR_SYSTEM_ENABLE_PGO_GENERATE` 与 `ERROR_SYSTEM_ENABLE_PGO_USE` 不可同时启用，否则 CMake 报错。警告包含 `-Wall -Wextra -Wpedantic -Wshadow -Wconversion -Wsign-conversion -Wdouble-promotion -Wnull-dereference -Wformat=2 -Wnon-virtual-dtor`（MSVC `/W4`）。
