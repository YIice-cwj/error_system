...
单条指令完成字段提取，100% 避免位域未定义行为。Reserved 3 bits 实为语义位：bit0=`retryable`、bit1=`transient`、bit2 预留，默认 0 兼容历史，`is_retryable()` / `is_transient()` / `set_retryable()` / `set_transient()` 直接读写。
...
| 模式 | 触发时机 | 满时策略 | 适用场景 |
|------|------|------|------|
| `sync` | 构造时同步调用 | 无丢失风险 | 日志 / 指标 |
| `async_queue` | 入队后台消费 | 拒绝 | 网络上报 / 持久化 |
| `sync_deferred` | 显式 `flush_deferred_notifications()` | 丢弃 | 请求批处理 / 事务边界累积 |

选择决策详见 [决策树 · 1](decision_tree.md#1-通知模式选择)。
...
原 `error_config_t` 与 `error_context_t` 承担过多职责，按单一职责原则拆分（详见上方 Config / Core 层职责表）。`error_config.h` 仅作向后兼容的统一包含入口，新代码直接包含细分头文件。
...
`DEFINE_ERROR_CODE` 宏结合 `constexpr error_code_t` 常量与 `inline const error_registrar_t` 静态初始化器，在 `main()` 前完成注册，避免跨 TU 静态初始化顺序问题（SIOF）。单例用 `std::call_once` + 函数局部静态，跨 TU 安全；但请勿在其它 TU 的静态初始化代码中查询注册表。宏签名详见 [Core 层 API · DEFINE_ERROR_CODE](api/core.md#define_error_code)。
...
- `i18n_t` 单例维护 `locale_t → (code identity → message)` 两级哈希，查询路径：active → default → 空字符串
- `subsystem_module_catalog_t` 维护 `(subsystem_id, module_id) → (locale → 名称)` 两级映射，与 `i18n_t` 共用 `locale_t` 但独立存储，避免双源不同步
...
`status_mapper_t` 为纯函数工具类，`to_http_status` / `to_grpc_status` 均 `constexpr`。映射策略：
...
| info / warn | 200 | OK |
| error·application | 400 | — |
| error·third_party | 502 | — |
| error·database / middleware | 503 | — |
| error·system / none | 500 | — |
| fatal | 500 | INTERNAL |
...
> **注意**：`ERROR_SYSTEM_ENABLE_PGO_GENERATE` 与 `ERROR_SYSTEM_ENABLE_PGO_USE` 不可同时启用，否则 CMake 报错。警告包含 `-Wall -Wextra -Wpedantic -Wshadow -Wconversion -Wsign-conversion -Wdouble-promotion -Wnull-dereference -Wformat=2 -Wnon-virtual-dtor -Wunused -Wunused-parameter -Wunused-variable -Wunused-function`（MSVC `/W4`）。

---

## 🧪 测试架构

- **框架**：GoogleTest v1.14.0（`FetchContent`）+ `gtest_discover_tests` 注册到 CTest
- **单元测试**：`tests/` 镜像 `include/` 结构，每个模块独立 `*_test.cc`，共 **21 个测试文件 / 481 个用例**，链接 `error_system::error_system` + `GTest::gtest_main`，仅应用警告选项不应用 LTO/PGO/Sanitizer
- **性能基准**：`tests/perf/` 共享 `perf_common.h` + 8 个场景文件（基线/仅栈追踪/仅插件/栈追踪+插件/全特性/错误路由插件/去重采样器/i18n 翻译模块）
- **代码生成**：Python3 脚本从 `config/errors/*.json` 生成错误码头文件、O(1) 字典与文档
