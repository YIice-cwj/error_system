...
- **线程安全访问**：`plugin_registry_t` 使用 `std::shared_mutex` 保护插件列表，`notify_error()` 采用快照模式——在读锁内复制插件列表，锁外执行回调
...
| 模块 | 测试文件数 | 测试用例数 | 测试文件 |
|------|-----------|-----------|----------|
| Core 层 | 7 | 70+ | `tests/core/*_test.cc` |
...
│   ├── error_context_test.cc     # error_context_t 测试
...
├── domain/                        # Domain 层测试
│   └── system_domain_test.cc     # system_domain_t 测试
└── perf/                          # 性能基线测试
    ├── error_context_benchmark.cc
    ├── error_registry_benchmark.cc
    └── plugin_registry_benchmark.cc
```
