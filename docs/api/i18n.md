...
> 💡 使用 `unordered_map<locale_t, unordered_map<code_identity_t, message_t>>` 两级哈希。`shared_mutex` 读多写少场景，`std::atomic<uint8_t>` 切换 active locale（哨兵值 255 表示未设置）。
...
public:
    ...
    // 查询
    [[nodiscard]] std::string_view get_message(locale_t locale, error_code_t code) const noexcept;  // 显式 locale
    [[nodiscard]] std::string_view get_message(error_code_t code) const noexcept;                    // active locale
...
1. active locale 命中 → 返回 active locale 的消息
