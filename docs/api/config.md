...
原 `error_config_t` 已按单一职责原则（SRP）拆分为四个独立配置类，`error_config.h` 仅作向后兼容的统一包含入口。新代码请直接包含对应的细分头文件。

---

## feature_flags_t

特性开关配置类 — 管理编译期特性开关与运行时布尔标志位。所有特性开关编译期决议，分支由编译器死代码消除。类仅包含静态成员，禁止实例化。
...
文本/i18n 输出开关已统一迁移到 `i18n_config_t::set_enable_i18n`，`feature_flags_t` 不再保留 `set_enable_text_output`。若编译期未启用对应特性，相关 `set_*` 调用无实际操作，`is_*_enabled()` 始终返回 `false`，编译器死代码消除未启用分支。
...
> **注意**：per-code 覆盖不影响全局 `is_stacktrace_enabled()` 判断；编译期未启用堆栈追踪时，所有 `set_*` 调用无操作，`get_*` 返回 `warn` / `std::nullopt`。

---

## formatter_config_t

自定义格式化器配置类 — 管理错误上下文的自定义格式化函数。自定义格式化器在 `error_context_serializer_t::to_string()` 内部被调用，若返回非空则替换默认文本输出。
...
`sync`（默认，同步阻塞）、`async_queue`（后台线程消费，队列满时拒绝）、`sync_deferred`（线程本地缓冲 + 显式 flush，请求边界批处理）三种通知模式的选择决策树详见 [决策树 · 1](../decision_tree.md#1-通知模式选择)。
...
CMake 编译期宏（`ERROR_SYSTEM_ENABLE_STACKTRACE` / `VALIDATION` / `LOCATION`）的完整选项与构建说明详见 [架构设计 · 编译配置](../architecture.md#编译配置)。
