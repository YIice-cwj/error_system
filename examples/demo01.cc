...
int main() {
    ...
    error_context_t ctx1{located_code_t{biz::trade_errors::ERR_ORDER_NOT_FOUND}, "订单查询失败"};
    ctx1.with("user_id", "8848").with("action", "query_order");
    std::cout << ctx1 << std::endl;

    std::cout << "\n--- 2. 不同等级的错误 ---" << std::endl;
    error_context_t ctx2{located_code_t{biz::user_errors::ERR_TOKEN_EXPIRED}, "Token 已过期"};
    std::cout << "Info 级别: " << ctx2 << std::endl;

    error_context_t ctx3{located_code_t{biz::payment_errors::ERR_ACCOUNT_FROZEN}, "账户已被冻结"};
    ...
    error_context_t root{located_code_t{infra::redis_errors::ERR_KEY_NOT_FOUND}, "Redis 键不存在"};
    error_context_t wrapper{located_code_t{biz::trade_errors::ERR_ORDER_NOT_FOUND}, "订单服务不可用"};
    ...
    error_context_t ctx4{located_code_t{biz::payment_errors::ERR_INSUFFICIENT_BALANCE}, "余额不足"};
    ctx4.with_batch({
        {"user_id", "8848"},
        {"balance", "150"},
        {"required", "500"},
    ...
    error_context_t ctx5{located_code_t{biz::payment_errors::ERR_INSUFFICIENT_BALANCE}, "余额不足"};
    ...
    error_context_t ctx_a{located_code_t{biz::trade_errors::ERR_ORDER_NOT_FOUND}, "相同消息"};
    error_context_t ctx_b{located_code_t{biz::trade_errors::ERR_ORDER_NOT_FOUND}, "相同消息"};
