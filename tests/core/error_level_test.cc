...
namespace error_system::core {
    ...
    TEST(error_level_test, to_int_converts_correctly) {
    ...
    TEST(error_level_test, from_int_converts_correctly) {
    ...
    TEST(error_level_test, from_int_invalid_returns_fatal) {
        EXPECT_EQ(from_int(5), error_level_t::fatal);
        EXPECT_EQ(from_int(255), error_level_t::fatal);
    }

    TEST(error_level_test, is_valid_checks_correctly) {
    ...
    TEST(error_level_test, from_string_converts_correctly) {
    ...
    TEST(error_level_test, from_string_invalid_returns_info) {
        EXPECT_EQ(from_string("unknown"), error_level_t::info);
        EXPECT_EQ(from_string(""), error_level_t::info);
    }

    TEST(error_level_test, to_string_converts_correctly) {
    ...
    TEST(error_level_test, next_level_returns_correct_value) {
    ...
    TEST(error_level_test, prev_level_returns_correct_value) {
    ...
    TEST(error_level_test, should_log_compares_correctly) {
    ...
    TEST(error_level_test, constexpr_evaluation_works) {
