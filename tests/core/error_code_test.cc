...
namespace error_system::core {
    ...
    TEST_F(error_code_test, get_module_group_id_extracts_correct_bits) {
        ...
        EXPECT_NE(group_id, 0);
        EXPECT_EQ(group_id & 0xFFFF, 0);
    }
    ...
    TEST_F(error_code_test, constexpr_evaluation_works) {
        ...
        static_assert(code.get_code() == 0x1234, "constexpr evaluation failed");
        EXPECT_EQ(code.get_code(), 0x1234);
    }
