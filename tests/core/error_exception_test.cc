...
namespace error_system::core {
    ...
    TEST_F(error_exception_test_t, exception_stores_context) {
        auto code = error_code_t(error_level_t::error, domain::system_domain_t::none, subsystem_id_t{0}, module_id_t{0}, error_number_t{42});
    ...
    TEST_F(error_exception_test_t, move_context_into_exception) {
        auto code = error_code_t(error_level_t::error, domain::system_domain_t::none, subsystem_id_t{0}, module_id_t{0}, error_number_t{200});
        error_registry_t::instance().register_error(code, "ERR_200", "Error 200");

        error_context_t context(located_code_t{code}, "moved");
