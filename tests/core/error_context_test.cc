...
namespace error_system::core {
    namespace {
        error_code_t make_code(uint16_t number) noexcept {
            return error_code_t(error_level_t::error, domain::system_domain_t::database, subsystem_id_t{9}, module_id_t{9}, error_number_t{number});
        }
    ...
    class error_context_test_t : public ::testing::Test {
        ...
        void SetUp() override {
            ...
            config::i18n_config_t::set_enable_i18n(true);
            ...
            catalog.register_subsystem_module(registered_code_.get_subsys(), registered_code_.get_module(), "perf", "context");
    ...
    TEST_F(error_context_test_t, with_string_view_inserts_payload_without_extra_overload_work) {
        error_context_t context(located_code_t{registered_code_}, "payload test");
    ...
    TEST_F(error_context_test_t, with_multiple_types_inserts_payload) {
        error_context_t context(located_code_t{registered_code_}, "multi type test");
    ...
    TEST_F(error_context_test_t, serialization_contains_registered_metadata_and_payload) {
        error_context_t context(located_code_t{registered_code_}, "serialize me");
    ...
    TEST_F(error_context_test_t, to_binary_includes_cause_chain) {
        ...
        error_context_t ctx_no_cause(located_code_t{registered_code_}, "wrapper error");
    ...
    TEST_F(error_context_test_t, to_binary_without_cause_has_zero_flag) {
        error_context_t context(located_code_t{registered_code_}, "no cause");
    ...
    TEST_F(error_context_test_t, with_batch_adds_multiple_payload_items) {
        error_context_t context(located_code_t{registered_code_}, "batch test");
        context.with_batch({
            {"host", "192.168.1.1"},
            {"port", "3306"},
            {"db", "users"}
    ...
    TEST_F(error_context_test_t, is_error_for_registered_error_code) {
        error_context_t context(located_code_t{registered_code_}, "error status");
        EXPECT_TRUE(context.is_error());
        EXPECT_FALSE(context.is_success());
    }

    TEST_F(error_context_test_t, is_success_for_success_code) {
        error_context_t context(located_code_t{error_code_t::make_success()}, "success status");
    ...
    TEST_F(error_context_test_t, payload_overflow_switches_to_unordered_map) {
        error_context_t context(located_code_t{registered_code_}, "overflow test");
        context.with("k1", "v1")
               .with("k2", "v2")
               .with("k3", "v3")
               .with("k4", "v4")
    ...
    TEST_F(error_context_test_t, equality_same_code_and_message) {
        error_context_t context_a{located_code_t{registered_code_}, "same message"};
        error_context_t context_b{located_code_t{registered_code_}, "same message"};
    ...
    TEST_F(error_context_test_t, inequality_different_message) {
        error_context_t context_a{located_code_t{registered_code_}, "message A"};
        error_context_t context_b{located_code_t{registered_code_}, "message B"};
    ...
    TEST_F(error_context_test_t, equality_considers_payload) {
        error_context_t context_a{located_code_t{registered_code_}, "msg"};
        context_a.with("key", "val");

        error_context_t context_b{located_code_t{registered_code_}, "msg"};
        context_b.with("key", "val");

        error_context_t context_c{located_code_t{registered_code_}, "msg"};
    ...
    TEST_F(error_context_test_t, deep_cause_chain_multiple_levels) {
        error_context_t root{located_code_t{registered_code_}, "root cause"};
    ...
    TEST_F(error_context_test_t, get_payload_value_returns_existing_key) {
        error_context_t context{located_code_t{registered_code_}, "test"};
    ...
    TEST_F(error_context_test_t, payload_size_and_is_payload_empty_reflect_state) {
        error_context_t context{located_code_t{registered_code_}, "size test"};
    ...
    TEST_F(error_context_test_t, for_each_payload_visits_sso_and_overflow_items) {
        error_context_t context{located_code_t{registered_code_}, "for_each test"};
        context.with("a", "1")
           .with("b", "2")
           .with("c", "3")
           .with("d", "4")
    ...
    TEST_F(error_context_test_t, for_each_payload_on_empty_context_does_nothing) {
        error_context_t context{located_code_t{registered_code_}, "empty"};
        size_t count = 0;
    ...
    TEST_F(error_context_test_t, wrap_move_takes_ownership_of_cause) {
        error_context_t cause{located_code_t{registered_code_}, "original cause"};
    ...
    TEST_F(error_context_test_t, to_json_includes_cause_chain_field) {
        error_context_t cause{located_code_t{registered_code_}, "root failure"};
    ...
    TEST_F(error_context_test_t, from_binary_round_trip_preserves_code_and_message) {
        error_context_t original(located_code_t{registered_code_}, "binary round trip");
    ...
    TEST_F(error_context_test_t, from_binary_round_trip_preserves_payload) {
        error_context_t original(located_code_t{registered_code_}, "with payload");
    ...
    TEST_F(error_context_test_t, from_binary_invalid_magic_returns_nullopt) {
        std::string bad = error_context_serializer_t::to_binary(error_context_t{located_code_t{registered_code_}, "x"});
        /** 破坏魔数 */
        bad[0] = '\x00';
    ...
    TEST_F(error_context_test_t, from_binary_truncated_data_returns_nullopt) {
        std::string blob = error_context_serializer_t::to_binary(error_context_t{located_code_t{registered_code_}, "x"});
    ...
    TEST_F(error_context_test_t, from_json_round_trip_preserves_code_and_message) {
        error_context_t original(located_code_t{registered_code_}, "json round trip");
    ...
    TEST_F(error_context_test_t, from_json_round_trip_preserves_payload) {
        error_context_t original(located_code_t{registered_code_}, "json payload");
    ...
    TEST_F(error_context_test_t, from_json_trailing_data_returns_nullopt) {
        error_context_t original(located_code_t{registered_code_}, "trail");
        const std::string json = error_context_serializer_t::to_json(original);
    ...
    TEST_F(error_context_test_t, from_binary_to_json_cross_format_code_consistent) {
        error_context_t original(located_code_t{registered_code_}, "cross format");
        original.with("k", "v");
        const std::string blob = error_context_serializer_t::to_binary(original);
    ...
    TEST_F(error_context_test_t, to_string_uses_custom_formatter_when_set) {
        error_context_t context(located_code_t{registered_code_}, "custom");
    ...
    TEST_F(error_context_test_t, to_string_custom_formatter_throws_falls_back) {
        error_context_t context(located_code_t{registered_code_}, "fallback");
        config::formatter_config_t::set_custom_formatter(
    ...
    TEST_F(error_context_test_t, from_binary_wrong_version_returns_nullopt) {
        error_context_t context(located_code_t{registered_code_}, "version");
    ...
    TEST_F(error_context_test_t, from_binary_trailing_data_returns_nullopt) {
        error_context_t context(located_code_t{registered_code_}, "trail");
        std::string blob = error_context_serializer_t::to_binary(context);
        blob.push_back('\x00');
        EXPECT_FALSE(error_context_serializer_t::from_binary(blob).has_value());
    }
